 ...
 */
contract OopSssGame is Ownable, Pausable {
    OopSssToken public immutable oopSssToken;
    ...
    constructor(address _oopSssToken) {
        oopSssToken = OopSssToken(_oopSssToken);
    ...
    function startGame() external whenNotPaused {
        require(oopSssToken.balanceOf(msg.sender) >= ENTRY_FEE, "Insufficient balance");
        
        // Pay entry fee
        oopSssToken.payEntryFee();
    ...
    function endGame(uint256 gameId, uint256 survivalTime) external whenNotPaused {
        ...
        // Award tokens
        if (reward > 0) {
            oopSssToken.awardSurvivalReward(msg.sender, survivalTime);
        }
